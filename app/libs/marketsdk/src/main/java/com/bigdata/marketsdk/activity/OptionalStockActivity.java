...
public class OptionalStockActivity extends BaseActivity {
    ...
    protected void baseCreate(Bundle savedInstanceState) {
        ...
        dataSave = new ListDataSave(OptionalStockActivity.this, ListDataSave.CODEFILE);
        gson = new Gson();

        boolean code = dataSave.getCodeState("code");
        if (code) {
            LoadDialog.show(OptionalStockActivity.this);
        }

        fenSHIModules = new ArrayList<>();


        adapter = new AbsAdapter<FenSHIModule>(OptionalStockActivity.this, fenSHIModules, R.layout.a_item_recylerview) {
            @Override
    ...
    private void initData() {
        ...
        HashMap<String, String> codes_map = dataSave.getDataMap(ListDataSave.CODES_MAP);
        for (String key : codes_map.keySet()) {
            codes.add(key);
        }
        ...
        if (codes.size() > 0) {
            ...
            adapter.notifyDataSetChanged();
            ToastUtils.showShortToast(OptionalStockActivity.this, "请添加个股");
            LoadDialog.dismiss(OptionalStockActivity.this);
        }
